...
* Communication with the core now happens via sockets. This means only a single process is spawned on startup and kept active throughout the lifetime of Atom.
  ...
  * Previously there also was a fix that prevented too many indexing requests from being sent. This was necessary because the previous indexing command might not have finished before the new one was sent and an additional process was spawned on each request. This fix was removed as multiple successive indexing events are simply queued and processed one by one due to the nature of socket communication with a single socket.
