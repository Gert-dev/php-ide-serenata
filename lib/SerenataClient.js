...
class SerenataClient extends AutoLanguageClient
{
    ...
    preInitialization(connection) {
        ...
        if (this.container.getConfiguration().get('annotations.enable')) {
    ...
    registerConfigListeners() {
        this.container.getConfiguration().onDidChange('annotations.enable', (value) => {
            if (value) {
                this.activateAnnotations();
            } else {
                this.deactivateAnnotations();
    ...
    setSnippetManager(snippetManager) {
        return this.getRefactoringProviders().forEach((provider) => {
    ...
    shouldStartForEditor(editor) {
        ...
        return this.container.getProjectManager().shouldStartForProject(projectPath);
    }
