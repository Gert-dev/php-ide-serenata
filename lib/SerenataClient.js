...
{
    constructor(proxy, projectManager, serenataServerManager, useStatementHelper, phpInvoker, config, packageName) {
        super();
        ...
        this.serenataServerManager = serenataServerManager;
    ...
    async installServerIfNecessary() {
        return new Promise((resolve, reject) => {
            if (this.serenataServerManager.isInstalled()) {
                resolve();
                return;
            }
    ...
    async installServer() {
        ...
        try {
            await this.serenataServerManager.install();
        } catch (e) {
            const message =
            ...
            atom.notifications.addError('Serenata - Server Installation Failed', {detail: message, dismissable: true});
        }
    ...
    notifyAboutSponsoringUnobtrusively() {
        ...
        let projectOpens = this.config.get('general.projectOpenCount');
        ...
        this.config.set('general.projectOpenCount', projectOpens);
    ...
    registerCommands() {
        ...
        atom.commands.add('atom-workspace', { 'php-ide-serenata:sort-use-statements': () => {
            ...
            if (activeTextEditor !== null) {
                this.useStatementHelper.sortUseStatements(activeTextEditor);
            }
        }});
    }
    ...
    testConfig() {
        ...
        const configTester = new ConfigTester(this.phpInvoker);

        atom.notifications.addInfo('Serenata - Testing Configuration', {
            dismissable: true,
            detail: 'Now testing your configuration... \n \n' +

                    `If you're running through a container, this may take a while the first time as the container ` +
        ...
        const callback = async () => {
            ...
            if (!wasSuccessful) {
                ...
                atom.notifications.addSuccess('Serenata - Success', {
                    dismissable: true,
                    detail: 'Your configuration is working correctly.'
                });
            }
        };
