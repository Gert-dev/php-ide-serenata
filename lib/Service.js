...
(Service = (function() {
    Service = class Service {
        ...
        */
        ...
        /**
         * Retrieves the types that are being used (called) at the specified location in the buffer. Note that this does
         * not guarantee that the returned types actually exist. You can use {@see getClassInfo} on the returned class
         * name to check for this instead.
         *
         * @param {TextEditor} editor            The text editor to use.
         * @param {Point}      bufferPosition    The cursor location of the item, such as the class member. Note that
         *                                       this should always be at the end of the actual member (i.e. just after
         *                                       it). If you want to ignore the element at the buffer position itself,
         *                                       see 'ignoreLastElement'.
         ...
         *                                       type (class) of 'b', as it is the last element, but as the user is
         *                                       still writing code, you may instead be interested in the type of
         *                                       'foo()' instead.
         *
         * @return {Promise}
         *
         * @example Invoking it on MyMethod::foo()->bar() will ask what class 'bar' is invoked on, which will whatever
         *          types foo returns.
        */
        ...
        /**
         * Retrieves the call stack of the function or method that is being invoked at the specified position. This can
         * be used to fetch information about the function or method call the cursor is in.
         *
         * @param {TextEditor} editor
         * @param {Point}      bufferPosition
         *
         * @return {Promise} With elements 'callStack' (array) as well as 'argumentIndex' which denotes the argument in
         *                   the parameter list the position is located at. Returns 'null' if not in a method or
         *                   function call.
         *
         * @example "$this->test(1, function () {},| 2);" (where the vertical bar denotes the cursor position) will
         *          yield ['$this', 'test'].
        */
        ...
        */
        getDocumentationUrlForClass(fqcn) {
            return this.config.get('php_documentation_base_urls').classes +
                this.getNormalizedFqcnDocumentationUrl(fqcn);
        }
        ...
        */
        getDocumentationUrlForFunction(fqcn) {
            return this.config.get('php_documentation_base_urls').functions +
                this.getNormalizedFqcnDocumentationUrl(fqcn);
        }
        ...
        */
        getDocumentationUrlForClassMethod(classlikeFqcn, name) {
            return this.config.get('php_documentation_base_urls').root +
                this.getNormalizedFqcnDocumentationUrl(classlikeFqcn) +
                '.' +
                this.getNormalizeMethodDocumentationUrl(name);
        }
