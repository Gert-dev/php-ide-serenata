...
class ParameterParser {
    ...
    */
    getRangeForCurrentScope(editor, bufferPosition) {
        ...
        for (
            ({ row } = bufferPosition), asc = bufferPosition.row <= 0;
            asc ? row <= 0 : row >= 0;
            asc ? row++ : row--
        ) {
            var asc1;
        ...
        for (
            ({ row } = startScopePoint), end = editor.getLineCount(), asc2 = startScopePoint.row <= end;
            asc2 ? row <= end : row >= end;
            asc2 ? row++ : row--
        ) {
            var asc3, end1;
            line = editor.lineTextForBufferRow(row);
            ...
            for (
                i = startIndex, end1 = line.length - 1, asc3 = startIndex <= end1;
                asc3 ? i <= end1 : i >= end1;
                asc3 ? i++ : i--
            ) {
                descriptions = editor.scopeDescriptorForBufferPosition(
    ...
    */
    getTypesForParameter(editor, parameter) {
        const successHandler = types => {
            ...
            const path = 'file://' + editor.getPath();
